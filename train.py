...
from joint_model import GCNCrowd
...
def matrix_completion_GCN(u_features, v_features, adj_train, train_labels, train_u_indices,
    ...
    if opt.testing:
        test_avg_loss, test_rmse, test_acc, response_test = sess.run([model.loss, model.rmse, model.accuracy, model.outputs_all],
    ...
    # pred_labels[diff < threshold] = -1
    pred_labels = pred_labels.reshape((num_users, num_items))
    # IPython.embed()
    return pred_labels, transform_onehot(pred_labels.T, num_users, len(class_values))


def train(opt):
    datasplit_path = "./data/dec/complete/"
    # datasplit_path = './data/sensor/balance/'
    fold = 1
    N_EPOCHS = 300
    BATCH_SIZE = 100

    u_features, v_features, responses, responses_mx, class_values, \
    true_class, rating_mx_train, train_labels, train_u_indices, train_v_indices, test_labels, test_u_indices, test_v_indices, \
        val_labels, val_u_indices, val_v_indices \
        = loadData(opt.dataset, opt.features, opt.data_seed, opt.testing, datasplit_path, opt.split_from_file, opt.verbose, fold, opt.test_rate)
    ...
    if opt.train_method == 'joint':
        placeholders, train_feed_dict, input_dim, num_users, num_items, num_support,\
            num_side_features, hidden_dim, responses_train = feed_dict_4_GCN(u_features, v_features, rating_mx_train, class_values,
                train_u_indices, train_v_indices, train_labels, self_connections=opt.self_connections, hidden_dim=opt.hidden[0],
                                                                             test_u_indices=test_u_indices, test_v_indices=test_v_indices, test_labels=test_labels,
                                                                             val_u_indices=val_u_indices, val_v_indices=val_v_indices, val_labels=val_labels)
        opt.hidden[0] = hidden_dim
        if len(np.unique(true_class)) > num_classes:
            true_class[true_class == 9] = 7
        ...
        sess = tf.Session()
        normal_op = optimizer.minimize(model.loss, global_step=global_step)
        ...
        for epoch in range(opt.epochs):
            # a = sess.run([loss, opt_op], feed_dict=train_feed_dict)
            outputs = sess.run([model.loss, model.basic_logits, opt_op, model.rmse], feed_dict=train_feed_dict)
            val_loss, val_pred, val_res = sess.run([model.loss, model.basic_logits, model.outputs_all], feed_dict=val_feed_dict)
        ...
        if True:
            ...
            predictY, weight, id_conf = IWMV(responses, class_values, dict())
            acc_iwmv_trn = accuracy_score(predictY, true_class)
            acc_iwmv_per_cls = accuracy_per_class(true_class, predictY)
            f1_iwmv = f1_score(true_class, predictY, average='macro')
            accuracy_trn = (acc_iwmv_trn, f1_iwmv, acc_iwmv_per_cls)
        ...
        if opt.matrix_completion == 'sparse':
            ...
            mx, responses_complete = matrix_completion_GCN(u_features, v_features, rating_mx_train, train_labels, train_u_indices,
                         train_v_indices, class_values, threshold)
            IPython.embed()
        elif opt.matrix_completion == 'full':
            res_train = rating_mx_train - 1
            res_train = res_train.T
            res_train = res_train.astype(np.int32)
        ...
        if opt.label_aggregation == 'iwmv':
            ...
            model.fit(v_features.toarray(), responses_complete, epochs=opt.epoch_bcn, shuffle=True, batch_size=BATCH_SIZE, verbose=2, validation_split=0.1)

            crowd_pred = model.predict(v_features.toarray())
            pred_labels = np.argmax(crowd_pred, axis=1)
            responses = transform_onehot(pred_labels, num_oracles, len(class_values))
            predictY, weight, id_conf = IWMV(responses, class_values, dict())
            acc_iwmv_trn = accuracy_score(predictY, true_class)
            acc_iwmv_per_cls = accuracy_per_class(true_class, predictY)
            f1_iwmv = f1_score(true_class, predictY, average='macro')
            ...
            # accuracy_tst = eval_model(model, x_test, y_test)
