...
ap.add_argument("-d", "--dataset", type=str, default="dec_appendicitis",
                choices=['ml_100k', 'ml_1m', 'ml_10m', 'douban', 'yahoo_music', 'flixster','sensor_rice','dec_appendicitis'],
                help="Dataset string.")
...
if DATASET == 'ml_1m' or DATASET == 'ml_10m':
    ...
    if FEATURES:
        datasplit_path = 'data/sensor/sensor_rice.pickle'
    else:
        datasplit_path = 'data/sensor/sensor_rice_nofeat.pickle'
elif DATASET[:3] == 'dec':
    datasplit_dir = 'data/dec/'
    if FEATURES:
        datasplit_path = 'data/dec/'+DATASET+'.pickle'
    else:
        datasplit_path = 'data/dec/'+DATASET+'_nofeat.pickle'
elif FEATURES:
...
if DATASET == 'flixster' or DATASET == 'douban' or DATASET == 'yahoo_music':
    ...
    u_features, v_features, adj_train, train_labels, train_u_indices, train_v_indices, \
        ...
        class_values, res_matrix_all, u_part2all, v_part2all = load_data_sensor(DATASET, DATASEED, TESTING,
                                                                                 datasplit_path, SPLITFROMFILE,
                                                                                 VERBOSE)
elif DATASET[:3] == 'dec':
    u_features, v_features, adj_train, train_labels, train_u_indices, train_v_indices, \
        val_labels, val_u_indices, val_v_indices, test_labels, test_u_indices, test_v_indices, \
        class_values = load_data_crowd(DATASET, FEATURES, DATASEED, TESTING, datasplit_dir, SPLITFROMFILE,
                                                                            VERBOSE)
...
for epoch in range(NB_EPOCH):
    ...
    #outs = sess.run([model.training_op, model.loss, model.rmse, model.accuracy, model.outputs, model.labels, model.outputs_all], feed_dict=train_feed_dict)
    outs = sess.run([model.training_op, model.loss, model.rmse, model.accuracy, model.outputs_all], feed_dict=train_feed_dict)

    train_avg_loss = outs[1]
    train_rmse = outs[2]

    train_acc = outs[3]
    outputs_observed = outs[4]  # |observed response in train| x No. class
    #labels_observed = outs[5]   # |observed response in train|, true label 0,1,2,...
    #outputs_all = outs[6]  # No. user x No. item x No. class


    acc = 0.0

    if DATASET == "sensor_rice":
        '''
        u_part2all = [u_part2all[u] for u in u_part2all]
        v_part2all = [v_part2all[v] for v in v_part2all]
        ph = {
            'res_matrix_all': tf.sparse_placeholder(tf.int64, shape=np.array(res_matrix_all.shape, dtype=np.int64)),
            'outputs_pred': tf.sparse_placeholder(tf.float32, shape=np.array(outputs_all.shape, dtype=np.float32)),
            'u_part2all': tf.placeholder(tf.int64, shape=len(u_part2all)),
            'v_part2all': tf.placeholder(tf.int64, shape=len(v_part2all))}
        fd = dict()
        fd.update({ph['res_matrix_all']: res_matrix_all})
        fd.update({ph['outputs_pred']: outputs_all})
        fd.update({ph['u_part2all']: u_part2all})
        fd.update({ph['v_part2all']: v_part2all})
    
        acc = sess.run(accuracy_res, feed_dict = fd)
        '''
        acc = accuracy_res_sensor(res_matrix_all, outputs_all, u_part2all, v_part2all)

    #elif DATASET == "dec_appendicitis":
        #acc = accuracy_res(labels_observed, outputs_observed)
