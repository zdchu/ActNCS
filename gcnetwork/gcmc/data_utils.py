...
### sensor_pickle => new sensor pickle
def create_data_sensor(dataset, seed=1234, verbose=True):
    sep = r','
    files = ['response.csv', 'instance.csv', 'oracle.csv', 'response_all.csv']

    if dataset == "sensor_rice":
        data_dir = 'data/sensor/'
    elif dataset[:3] == "dec":
        data_dir = 'data/dec/'+dataset[4:]+'_'
    ...
    u_nodes_all = data_array[:, 0].astype(dtypes['oracle_id'])
    v_nodes_all = data_array[:, 1].astype(dtypes['instance_id'])
    ratings_all = data_array[:, 2].astype(dtypes['response'])

    u_nodes_all, u_dict_all, num_users_all = map_data(u_nodes_all) # u_dict: out -> in
    v_nodes_all, v_dict_all, num_items_all = map_data(v_nodes_all)


    u_part2all = {} # part inner index -> all inner index
    v_part2all = {}
    for ui in u_dict:
        u_part2all[u_dict[ui]] = u_dict_all[ui]
    for vi in v_dict:
        v_part2all[v_dict[vi]] = v_dict_all[vi]

    res_matrix_all = np.zeros((num_users_all, num_items_all))
    res_matrix_all = res_matrix_all.astype(np.int64)
    
    for i in range(len(u_nodes_all)):
        ui = u_nodes_all[i]
        vi = v_nodes_all[i]
        res_i = ratings_all[i]
        res_matrix_all[ui][vi] = res_i
    ...
    return num_users, num_items, u_nodes_ratings, v_nodes_ratings, ratings, u_features, v_features, res_matrix_all, \
