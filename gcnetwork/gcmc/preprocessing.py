...
### sensor
'''
    loads data from sensor dataset
'''
def load_data_sensor(dataset, seed=1234, testing=False, datasplit_path=None, datasplit_from_file=False,
                              verbose=True):
    if datasplit_from_file and os.path.isfile(datasplit_path):
        ...
        with open(datasplit_path) as f:
            num_users, num_items, u_nodes, v_nodes, ratings, u_features, v_features, res_matrix_all, u_part2all, v_part2all = pkl.load(f)
        ...
        num_users, num_items, u_nodes, v_nodes, ratings, u_features, v_features, res_matrix_all, u_part2all, v_part2all \
            = create_data_sensor(dataset, seed=seed, verbose=verbose)

        with open(datasplit_path, 'w') as f:
            pkl.dump([num_users, num_items, u_nodes, v_nodes, ratings, u_features, v_features, res_matrix_all,
                      u_part2all, v_part2all], f)
    ...
    num_test = int(np.ceil(ratings.shape[0] * 0.05))  #

    num_val = int(np.ceil(ratings.shape[0] * 0.95 * 0.05))
    ...
    return u_features, v_features, rating_mx_train, train_labels, u_train_idx, v_train_idx, \
        val_labels, u_val_idx, v_val_idx, test_labels, u_test_idx, v_test_idx, class_values, res_matrix_all, \
           u_part2all, v_part2all


### sensor
'''
    loads data from crowd
'''
def load_data_crowd(dataset, FEATURES, seed=1234, testing=False, datasplit_dir=None, datasplit_from_file=False,
                              verbose=True):
    if FEATURES:
    ...
    if datasplit_from_file and os.path.isfile(datasplit_path):
        ...
        num_users, num_items, u_nodes, v_nodes, ratings, u_features, v_features \
            = create_crowd_data(dataset, datasplit_dir, seed=seed, verbose=verbose)
    ...
    # number of test and validation edges
    num_test = int(np.ceil(ratings.shape[0] * 0.3))  #

    num_val = int(np.ceil(ratings.shape[0] * 0.7 * 0.2))
