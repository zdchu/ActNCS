...
class BilinearMixture(Layer):
    ...
    def _call(self, inputs):
        ...
        ###
        u_idx = [u for u in range(self.num_user) for v in range(self.num_item)]
        u_inputs = tf.gather(u_inputs, u_idx)
        v_idx = [v for u in range(self.num_user) for v in range(self.num_item)]
        v_inputs = tf.gather(v_inputs, v_idx)

        if self.user_item_bias:
            #u_bias = tf.gather(self.vars['user_bias'], self.u_indices[1:])
            #v_bias = tf.gather(self.vars['item_bias'], self.v_indices[1:])

            u_bias = tf.gather(self.vars['user_bias'], u_idx)
            v_bias = tf.gather(self.vars['item_bias'], v_idx)
